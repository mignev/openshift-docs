...
To verify that cluster is up and running, connect to a master instance, and run the following:
...
node hosts, and three node hosts and all of them are running. All hosts in the
cluster should be visible in this output.

The `Ready` status means that master hosts can communicate with node hosts and
that the nodes are ready to run pods, except nodes in which scheduling is
disabled.
...
All etcd hosts should containt the master host name if the `etcd` cluster is
...
The numbers in `DESIRED` and `CURRENT` columns should match the number of nodes
hosts.
