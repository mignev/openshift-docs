...
[NOTE]
====
Currently, a {product-title} project backup and restore tool is being developed
...
wouldn't exist in a restored environment. For instance, running the sample
...
[NOTE]
====
Some files can be empty if the objects doesn't exist when the export is
executed.
====
...
[WARNING]
====
The script doesn't support multiple container pods currently, use it
with caution.
====
...
[NOTE]
====
Some errors can appear as some objects as `serviceaccounts` and secrets
are created automatically when creating the project.
====
